...
Definition
~~~~~~~~~~
Defined in the header :file:`matrix_sparse.hpp`.
...
:`A`: The type of the adapted array. [#f2]_ Defaults to `map_std<std::size_t, T>`
...
:`F`: Functor describing the storage organization. [#f3]_ Defaults to `row_major`
:`IB`: The index base of the compressed vector. [#f4]_ Defaults to `0`
:`IA`: The type of the adapted array for indices. [#f5]_ Defaults to `unbounded_array<std::size_t>`
:`TA`: The type of the adapted array for values. [#f6]_ Defaults to `unbounded_array<T>`
...
Description
~~~~~~~~~~~
The templated class `coordinate_matrix<T, F, IB, IA, TA>` is the base container adaptor for compressed matrices. For a (m x n )-dimensional sorted coordinate matrix and `0 <= i < m`, `0 <= j < n` the non-zero elements mi, j are mapped via (i x n + j) for row major orientation or via (i + j x m) for column major orientation to consecutive elements of the index and value containers, i.e. for elements `k = mi1` ,`j1` and `k + 1 = mi2`, `j2` of the container holds `i1 < i2` or `(i1 = i2` and `j1 < j2`) with row major orientation or `j1 < j2` or (`j1 = j2` and `i1 < i2`) with column major orientation.
...
.. [#f2] Supported parameters for the adapted array are `map_array<std::size_t, T>` and `map_std<std::size_t, T>`. The latter is equivalent to `std::map<std::size_t, T>`.

.. [#f3] Supported parameters for the storage organization are row_major and column_major.

.. [#f4] Supported parameters for the index base are 0 and 1 at least.

.. [#f5] Supported parameters for the adapted array are unbounded_array<> , bounded_array<> and std::vector<> .
