...
:A: 	The type of the :ref:`storage-concept` array. [#f1]_ Defaults to `unbounded_array<T>`
...
.. [#f1] Common parameters for the Storage array are `unbounded_array<T>`, `bounded_array<T>` and `std::vector<T>`.
